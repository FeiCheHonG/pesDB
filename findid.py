...
def insert_base(player_num, urlID, player_name):
	"""向数据库中插入球员号, urlID, 球员名三项信息"""
	sql_insert = "INSERT INTO players (球员号, urlID, 球员名) VALUES (%d,'%s','%s')"
	try:
		cur.execute(sql_insert % (player_num, urlID, player_name))
		cur.connection.commit()
	except Exception as e:
		cur.connection.rollback()
		print('插入：',player_name,'时出错,urlID=',urlID)
...
def savepageinfo(players_num, players_urlid, players_name):
	"""存储页面中球员的urlID和姓名"""
	print("开始存储球员三项基本信息")
	#将三项基本数据数据插入数据库
	for i in range(len(players_urlid)):
		print(players_num[i], players_urlid[i], players_name[i])
		insert_base(players_num[i], players_urlid[i], players_name[i])
	print("存储完毕！")
...
savepageinfo(players_num, players_urlid, players_name)

for i in range(len(players_urlid)):
	level_data, other_info = [], []
